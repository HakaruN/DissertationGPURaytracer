...
unsigned long long numTriangles = 100000;
...
void D3D12HelloTriangle::OnInit()
{
	...
	//load assets ususll opens a command list, loads stuff and closes the command list,
	//we want to init RT here so we need the command still open when LoadAssets returns
	//therefore we will close the command list here
...
void D3D12HelloTriangle::PopulateCommandList()
{
	...
	// Record commands.
	// #DXR - //if we are rsterising then do this
	if (m_raster)
	{
		// #DXR Extra: Perspective Camera
...
void D3D12HelloTriangle::checkRaytracingSupport()
{
	...
	if (options5.RaytracingTier < D3D12_RAYTRACING_TIER_1_0)//if no rt support, throw exteption
		throw std::runtime_error("Raytracing not supported on device");
}


//Creating Accel structs



//Create a BLAS based on a list of vertex
...
D3D12HelloTriangle::AccelerationStructureBuffers D3D12HelloTriangle::CreateBottomLevelAS(std::vector<std::pair<ComPtr<ID3D12Resource>, uint32_t>> vVertexBuffers)
{
	...
	// Adding all vertex buffers, this does not transform their position.
	...
	D3D12HelloTriangle::AccelerationStructureBuffers buffers;
...
void D3D12HelloTriangle::CreateTopLevelAS(
	...
	& instances // pair of bottom level AS pointer and transform matrix of the instance
)
{
...
{
	//call the CreateBottomLevelAS to build the BLAS
	AccelerationStructureBuffers bottomLevelBuffers =
		CreateBottomLevelAS({ {m_vertexBuffer.Get(), 3} });

	// Just one instance for now
	/*
	float x = ((rand() % 100) - 50) / 10;
	float y = ((rand() % 100) - 50) / 10;
	float z = ((rand() % 100) - 50) / 10;
	m_instances = { {bottomLevelBuffers.pResult,XMMatrixTranslation(x, y, z + 20)} };
	*/
	for (unsigned long long i = 0; i < numTriangles; i++)//make num triangles -1
	{
		...
		//far triangles
		//float x = ((rand() % 200) - 100) / 10;
		//float y = ((rand() % 100) - 50) / 10;
		//float z = ((rand() % 150)) + 10;
		float x = ((rand() % 300) - 150) / 10;
		float y = ((rand() % 300) - 150) / 10;
		float z = ((rand() % 300) - 150) / 10;
		m_instances.push_back({ bottomLevelBuffers.pResult, XMMatrixTranslation(x, y, z) });
	}
...
//create a hit root sig
// The hit shader communicates only through the ray payload, and therefore does
// not require any resources
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreateHitSignature() {
	nv_helpers_dx12::RootSignatureGenerator rsc;
	rsc.AddRootParameter(D3D12_ROOT_PARAMETER_TYPE_SRV);//add a shader resource view to the hit sig
	return rsc.Generate(m_device.Get(), true);
}

//Create a miss root sig
...
void D3D12HelloTriangle::CreateShaderResourceHeap()
{
	...
	// raytracing output and 1 SRV for the TLAS and one for the camera buffer
...
void D3D12HelloTriangle::CreateShaderBindingTable()
{
  ...
  // struct is a UINT64, which then has to be reinterpreted as a pointer.
	...
	m_sbtHelper.AddHitGroup(L"HitGroup", { (void*)(m_vertexBuffer->GetGPUVirtualAddress()) });//pass in the vertex buffer to the hit group
